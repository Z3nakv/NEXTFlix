...
export default function Footer() {
  ...
  useEffect(() => {
    const lazyFooterRef = footerRef.current;
    ...
    if (lazyFooterRef) {
      observer.observe(lazyFooterRef);
    }

    return () => {
      if (lazyFooterRef) {
        observer.unobserve(lazyFooterRef);
      }
    };
  }, []);

    return (
        <footer ref={footerRef} className="flex flex-col justify-self-end w-full text-white" aria-labelledby="footer-heading">
            {visible && (
            <>
